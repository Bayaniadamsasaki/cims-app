            ...
            <div className="space-y-6">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 relative z-10">
                        ...
                        <div className="flex items-center space-x-3">
                            <div className="bg-brand-bg/80 border border-brand-border px-3.5 py-2 rounded-xl text-xs font-mono">
                                ...
                                <span className="text-cyan-700 font-bold">{ruijieConfig?.appId || "open2a30c702449b"}</span>
                            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-brand-card border border-brand-border p-5 rounded-2xl hover:border-cyan-500/40 transition">
                        ...
                        <div className="text-xs text-cyan-700 mt-1 font-mono">Managed via Ruijie Cloud</div>
                    </div>
                    ...
                    <div className="bg-brand-card border border-brand-border p-5 rounded-2xl hover:border-emerald-500/40 transition">
                        ...
                        <div className="text-3xl font-extrabold text-emerald-700 mt-2">{summary?.onlineDevices || 0}</div>
                        <div className="text-xs text-emerald-700/80 mt-1 font-mono">Active & Transmitting</div>
                    </div>

                    <div className="bg-brand-card border border-brand-border p-5 rounded-2xl hover:border-purple-200 transition">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="text-3xl font-extrabold text-purple-700 mt-2">{wirelessClients.length}</div>
                        <div className="text-xs text-purple-700 mt-1 font-mono">Connected STAs</div>
                    </div>
                    ...
                    <div className="bg-brand-card border border-brand-border p-5 rounded-2xl hover:border-amber-500/40 transition">
                        ...
                        <div className="text-3xl font-extrabold text-amber-700 mt-2">{alarms.length}</div>
                        <div className="text-xs text-amber-700 mt-1 font-mono">Active Events</div>
                    </div>
                </div>
                ...
                {connection?.hint && (
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 flex items-start space-x-3 text-amber-700">
                        ...
                        <div className="space-y-1 text-xs leading-relaxed">
                            <div className="font-bold text-amber-700 text-sm flex items-center space-x-2">
                                <span>Langkah Penting Otorisasi Ruijie Cloud API (Code 5 - Permission Required)</span>
                            </div>
                            <p className="text-amber-700/90">
                                APPID <code className="bg-amber-950/60 px-1.5 py-0.5 rounded font-mono text-amber-700">{connection?.app_id}</code> sudah benar dan diterima oleh Ruijie Cloud, namun **perlu di-Otorisasi (Authorize)** di dalam akun/project Ruijie Cloud Anda:
                            </p>
                            <ol className="list-decimal list-inside space-y-1 text-amber-700/80 pt-1 font-mono">
                                <li>Buka Portal Ruijie Cloud (<a href="https://cloud-as.ruijienetworks.com" target="_blank" rel="noreferrer" className="underline text-cyan-700 font-bold">cloud-as.ruijienetworks.com</a> atau <a href="https://cloud.ruijienetworks.com" target="_blank" rel="noreferrer" className="underline text-cyan-700 font-bold">cloud.ruijienetworks.com</a>)</li>
                                <li>Masuk ke Project Jaringan Anda ➔ Klik <strong>Settings</strong> ➔ <strong>Open API / Authorize APPID</strong></li>
                                <li>Masukkan APPID: <strong className="text-slate-900">{connection?.app_id}</strong> lalu klik <strong>Authorize Project Access</strong></li>
                            </ol>
                        </div>
                    </div>
                )}
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-brand-card border border-brand-border p-4 rounded-2xl">
                            <div className="relative flex-1">
                                <input
                                    ...
                                    className="w-full bg-brand-bg border border-brand-border rounded-xl px-4 py-2 text-sm text-slate-900 placeholder-brand-textSecondary focus:outline-none focus:border-cyan-500"
                                />
                            </div>
                                ...
                                {["ALL", "Access Point", "Switch", "Gateway"].map((type) => (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
                                            filterType === type
                                                ? "bg-cyan-500/20 text-cyan-700 border border-cyan-500/40"
                                                : "bg-brand-bg text-brand-textSecondary border border-brand-border hover:text-slate-900"
                                        }`}
                                    >
                                        {type}
                                    </button>
                                        ...
                                        {filteredDevices.length === 0 ? (
                                            ...
                                            filteredDevices.map((dev, idx) => (
                                                <tr key={idx} className="hover:bg-brand-cardElevated/50 transition">
                                                    ...
                                                    <td className="px-6 py-4 font-mono text-xs font-semibold text-cyan-700">{dev.sn}</td>
                                                    <td className="px-6 py-4">
                                                        <div className="font-mono text-xs font-bold text-slate-900">{dev.ip}</div>
                                                        <div className="font-mono text-[11px] text-brand-textSecondary">{dev.mac}</div>
                                                    </td>
                                                    <td className="px-6 py-4 text-xs font-medium text-slate-500">{dev.type}</td>
                                                    <td className="px-6 py-4">
                                                        <span
                                                    ...
                                                    <td className="px-6 py-4 font-mono font-bold text-purple-700">{dev.client_count} STAs</td>
                                                    <td className="px-6 py-4">
                ...
                {activeTab === "clients" && (
                    <div className="bg-brand-card border border-brand-border rounded-2xl overflow-hidden shadow-lg">
                        <div className="p-5 border-b border-brand-border flex items-center justify-between">
                            <h3 className="font-bold text-slate-900 text-base">Connected Wi-Fi Clients (STAs)</h3>
                            <span className="text-xs text-cyan-700 font-mono font-bold">{wirelessClients.length} Active Sessions</span>
                        </div>
                            ...
                            <table className="w-full text-left text-sm text-brand-textSecondary">
                                ...
                                <tbody className="divide-y divide-brand-border">
                                    {wirelessClients.map((client, idx) => (
                                        <tr key={idx} className="hover:bg-brand-cardElevated/50 transition">
                                            <td className="px-6 py-4">
                                                <div className="font-bold text-slate-900">{client.hostname}</div>
                                                <div className="font-mono text-xs text-brand-textSecondary">{client.mac}</div>
                                            </td>
                                            <td className="px-6 py-4 font-mono text-xs font-bold text-cyan-700">{client.ip}</td>
                                            <td className="px-6 py-4 font-semibold text-emerald-700 text-xs">{client.ssid}</td>
                                            <td className="px-6 py-4 text-xs font-medium text-slate-900">{client.ap_name}</td>
                                            <td className="px-6 py-4 font-mono text-xs">
                                                <span className={getRssiColor(client.rssi)}>{client.rssi} dBm</span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="text-xs text-purple-700 font-semibold">{client.band}</div>
                ...
                {activeTab === "alarms" && (
                    <div className="bg-brand-card border border-brand-border rounded-2xl overflow-hidden shadow-lg">
                        <div className="p-5 border-b border-brand-border flex items-center justify-between">
                            <h3 className="font-bold text-slate-900 text-base">Cloud Security Alarms & System Logs</h3>
                            <span className="text-xs text-amber-700 font-mono font-bold">{alarms.length} Events</span>
                        </div>
                            ...
                            {alarms.map((alarm, idx) => (
                                ...
                                >
                                    <div className="flex items-start space-x-3">
                                        ...
                                        <div>
                                            <div className="flex items-center space-x-2">
                                                <span className="font-bold text-slate-900 text-sm">{alarm.title}</span>
                                                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-700 border border-cyan-500/20">
                        ...
                        <div className="bg-brand-card border border-brand-border p-6 rounded-2xl space-y-4">
                            ...
                            <div className="space-y-3 text-sm font-mono">
                                <div className="p-3 bg-brand-bg rounded-xl border border-brand-border flex justify-between">
                                    <span className="text-brand-textSecondary">APPID:</span>
                                    <span className="text-cyan-700 font-bold">{ruijieConfig?.appId}</span>
                                </div>
                                <div className="p-3 bg-brand-bg rounded-xl border border-brand-border flex justify-between">
                                    <span className="text-brand-textSecondary">Base URL:</span>
                                    <span className="text-slate-900">{ruijieConfig?.baseUrl}</span>
                                </div>
                                <div className="p-3 bg-brand-bg rounded-xl border border-brand-border flex justify-between">
                                    <span className="text-brand-textSecondary">OAuth Endpoint:</span>
                                    <span className="text-purple-700 text-xs">/service/api/oauth20/client/access_token</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-brand-card border border-brand-border p-6 rounded-2xl space-y-4">
                            ...
                            <div className="space-y-3 text-sm font-mono">
                                <div className="p-3 bg-brand-bg rounded-xl border border-brand-border flex justify-between items-center">
                                    ...
                                    <span className="px-2.5 py-0.5 rounded text-xs font-bold bg-emerald-500/10 text-emerald-700 border border-emerald-500/30">
                                        SUCCESS
                                    </span>
                                </div>
                                <div className="p-3 bg-brand-bg rounded-xl border border-brand-border flex justify-between">
                                    <span className="text-brand-textSecondary">API Latency:</span>
                                    <span className="text-emerald-700 font-bold">{connection?.latency_ms} ms</span>
                                </div>
                                <div className="p-3 bg-brand-bg rounded-xl border border-brand-border flex justify-between">
                                    <span className="text-brand-textSecondary">Active Token:</span>
                                    <span className="text-cyan-700">{connection?.token || "ruijie_tk_..."}</span>
                                </div>
                            </div>
                        </div>
                    </div>
