...
export default function Rooms({ rooms = [], floors = [] }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (editingRoom) {
            post(route('rooms.update', editingRoom.id), {
                onSuccess: () => {
                ...
                <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/85 flex items-center justify-center p-4 backdrop-blur-md">
                    <div className="relative w-full max-w-md rounded-2xl bg-brand-card border border-brand-border p-6 shadow-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-xs font-semibold text-brand-textSecondary mb-1">Floor Location*</label>
                                <select
                                    required
                                    value={data.floor_id}
                                ...
                                >
                                    ...
                                    {floors.map((floor) => (
                                        <option key={floor.id} value={floor.id}>
                                            {floor.building?.name || 'Building'} - {floor.name} (Lvl {floor.level})
                                        </option>
                                    ))}
                                </select>
